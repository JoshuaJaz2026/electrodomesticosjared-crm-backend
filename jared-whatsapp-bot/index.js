...
const axios = require('axios'); // Nueva dependencia vital para llamadas HTTP a Meta
...
const app = express();
app.use(cors()); 
app.use(express.json()); // Vital para poder leer lo que nos manda Meta
...
// ==========================================
// 🛡️ WEBHOOK DE META (VERIFICACIÓN DE SEGURIDAD)
// ==========================================
...
// ==========================================
// 📥 WEBHOOK DE META (RECEPCIÓN DE MENSAJES)
// ==========================================
app.post('/webhook', async (req, res) => {
    ...
    if (body.object) {
        if (body.entry && body.entry[0].changes && body.entry[0].changes[0].value.messages && body.entry[0].changes[0].value.messages[0]) {
            ...
            let phone = msgObj.from; // Número del cliente
            ...
            try {
                await sql`INSERT INTO contacts (id, name, last_message, updated_at) VALUES (${contactId}, ${contactName}, ${text}, NOW()) ON CONFLICT (id) DO UPDATE SET last_message = ${text}, updated_at = NOW()`;
                ...
                io.emit('whatsapp-message', { id: messageId, from: contactId, contactName: contactName, summaryText: text, body: text, mediaUrl: null, mimeType: null, ack: 1, timestamp: new Date(), isMine: false });
            } catch (error) {
                console.error("Error BD recibiendo:", error);
            }
        }
        res.sendStatus(200); // Meta exige una respuesta rápida (200 OK) para no reenviar el mensaje
    } else {
        res.sendStatus(404);
    }
});
...
io.on('connection', (socket) => {
    // Al conectar ahora solo decimos "Estoy listo" porque no hay Chrome ni códigos QR
